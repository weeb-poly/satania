...
LABEL org.opencontainers.image.source https://github.com/weeb-poly/satania

# Create the working directory
WORKDIR /app

# Copy pex file from build-env
#COPY --from=build-env /app/satania-loop.pyz /app/
COPY --from=build-env /app/satania-once.pyz /app/

CMD ["./satania-once.pyz"]
